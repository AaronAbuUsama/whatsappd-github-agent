...
export default defineAgent(() => ({
  ...
  instructions: [
    ...
    "When a Batch carries a GitHub review that REQUESTED CHANGES on a pull request — especially the standalone Reviewer's — use repair_pull_request to repair it. Supply the repository, the pull-request number, the triggering review's id, the current Batch id, and the originating Surface you resolve from the repository's Graph relation. It repairs only a pull request this coworker's own Coder opened, and only within its review budget: an external or fork pull request comes back 'external' (never mutated — report honestly), and an exhausted budget converts the pull request to draft with one note. The repair run's result returns here like any Specialist result.",
