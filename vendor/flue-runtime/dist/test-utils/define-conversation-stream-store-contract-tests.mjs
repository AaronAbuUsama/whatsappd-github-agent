...
//#region src/test-utils/define-conversation-stream-store-contract-tests.ts
function userRecord(id, messageId) {
	return {
		v: 1,
		id,
		type: "user_message",
...
function ownedUserRecord(id, messageId, submissionId, attemptId) {
	return {
		...userRecord(id, messageId),
		submissionId,
		attemptId
	};
}
async function claimContractSubmission(executionStore, submissionId, attemptId) {
...
function defineConversationStreamStoreContractTests(label, backend) {
	describe(label, () => {
		let cleanup;
		...
		it("creates one stream when exact identities race", async () => {
			...
			await expect(Promise.all([stream.createStream("agents/echo/contract", identity), stream.createStream("agents/echo/contract", identity)])).resolves.toEqual([void 0, void 0]);
		...
		it("retains one identity when conflicting creates race", async () => {
			...
			const results = await Promise.allSettled([stream.createStream("agents/echo/contract", first), stream.createStream("agents/echo/contract", second)]);
		...
		it("appends only the exact reserved settlement while an attempt is terminalizing", async () => {
			...
			await expect(stream.append({
				...base,
				records: [record, userRecord("extra", "extra")]
			})).rejects.toThrow();
			await expect(stream.append({
				...base,
				records: [{
			...
			await expect(stream.append({
				...base,
				records: [record]
			})).resolves.toEqual({ offset: "0000000000000000_0000000000000000" });
		});
		...
		it("replays strictly after a batch offset", async () => {
			...
			expect(await stream.read("agents/echo/contract", { offset: first.offset })).toMatchObject({ batches: [{ records: [{ id: "record_2" }] }] });
		});
...
//#endregion
export { defineConversationStreamStoreContractTests };
