...
//#region src/test-utils/define-run-store-contract-tests.ts
function defineRunStoreContractTests(label, backend) {
	describe(label, () => {
		let cleanup;
		...
		it("preserves the existing record when createRun() replays a run id", async () => {
			...
			await store.createRun({
				...
				startedAt: "2026-06-01T10:00:00.000Z",
				input: { report: "weekly" },
				traceCarrier: { traceparent: "00-11111111111111111111111111111111-2222222222222222-01" }
			});
			await store.endRun({
				runId: "run_01DAILYREPORT",
			...
			await store.createRun({
				...
				traceCarrier: { traceparent: "00-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-bbbbbbbbbbbbbbbb-01" }
			});
			expect(await store.getRun("run_01DAILYREPORT")).toMatchObject({
				status: "completed",
				startedAt: "2026-06-01T10:00:00.000Z",
		...
		it("returns null when lookupRun() receives an unknown run id", async () => {
			expect(await (await create()).lookupRun("run_01MISSING")).toBeNull();
		});
		...
		it("filters run pointers when status or workflow name is requested", async () => {
			...
			expect((await store.listRuns({ status: "errored" })).runs.map((pointer) => pointer.runId)).toEqual(["run_02DAILYREPORT"]);
			expect((await store.listRuns({ workflowName: "daily-report" })).runs.map((pointer) => pointer.runId)).toEqual(["run_02DAILYREPORT", "run_01DAILYREPORT"]);
		});
		it("continues run listing when a cursor is supplied", async () => {
			...
			expect(firstPage.runs.map((pointer) => pointer.runId)).toEqual(["run_03DAILYREPORT", "run_02DAILYREPORT"]);
...
//#endregion
//#region src/test-utils/define-store-contract-tests.ts
/**
...
async function admitDispatchReady(store, input) {
	...
	return {
		kind: "submission",
		submission: await store.submissions.markSubmissionCanonicalReady(admission.submission.submissionId) ?? admission.submission
	};
}
...
*/
function defineStoreContractTests(label, backend) {
	describe(label, () => {
		let _cleanup;
		async function create() {
		...
		describe("dispatch admission", () => {
			it("admits one queued dispatch row when the same submission is replayed", async () => {
				...
				expect(await store.submissions.admitDispatch(dispatchInput())).toEqual(first);
			...
			it("returns conflict when one dispatch id is reused with another payload", async () => {
				...
				expect(await store.submissions.admitDispatch(dispatchInput({ input: { text: "Different" } }))).toEqual({ kind: "conflict" });
			});
		});
		describe("direct admission", () => {
			it("round-trips direct submission images", async () => {
				const store = await create();
				const input = directInput({ payload: {
					message: "Hello",
					images: [{
						type: "image",
						data: "image-data",
				...
				expect((await admitDirectReady(store, input)).input).toEqual(input);
			...
			it("rejects replay when a direct submission image has different bytes", async () => {
				const store = await create();
				await admitDirectReady(store, directInput({ payload: {
					message: "Hello",
					images: [{
						type: "image",
						data: "first-image",
				...
				await expect(admitDirectReady(store, directInput({ payload: {
					message: "Hello",
					images: [{
						type: "image",
						data: "second-image",
		...
		describe("canonical readiness", () => {
			it("does not list or claim a queued submission before canonical readiness", async () => {
				...
				expect(await store.submissions.claimSubmission(claim("dispatch-1", "attempt-1"))).toBeNull();
			});
			it("lists unready queued submissions in admission order", async () => {
				...
				const third = await store.submissions.admitDirect(directInput({
					submissionId: "direct-2",
					id: "agent-2"
				}));
				expect(await store.submissions.listUnreadySubmissions()).toEqual([expect.objectContaining({ submissionId: first.kind === "submission" ? first.submission.submissionId : "" }), expect.objectContaining({ submissionId: third.submissionId })]);
			});
			it("lists and claims a queued submission after canonical readiness", async () => {
				...
				expect(await store.submissions.claimSubmission(claim("dispatch-1", "attempt-1"))).toMatchObject({
					status: "running",
		...
		describe("submission ordering", () => {
			it("orders direct and dispatched submissions together within one session", async () => {
				...
				const other = await admitDirectReady(store, directInput({
					submissionId: "direct-2",
					id: "agent-2"
				}));
			...
			it("lists queued dispatches in admission order and selects one runnable head per session", async () => {
				...
				expect(await store.submissions.listRunnableSubmissions()).toEqual([expect.objectContaining({ submissionId: "dispatch-1" }), expect.objectContaining({ submissionId: "dispatch-3" })]);
			});
		});
		...
		describe("lifecycle transitions", () => {
			...
			it("ignores stale-attempt settlement and keeps the first owning terminal state", async () => {
				...
				await store.submissions.failSubmission(attempt("dispatch-1", "attempt-1"), /* @__PURE__ */ new Error("first failure"));
				await store.submissions.completeSubmission(attempt("dispatch-1", "attempt-1"));
				await store.submissions.failSubmission(attempt("dispatch-1", "attempt-1"), /* @__PURE__ */ new Error("later failure"));
				expect(await store.submissions.getSubmission("dispatch-1")).toMatchObject({
					status: "settled",
					error: "first failure"
				});
		...
		describe("session abort requests", () => {
			it("stamps an abort intent on a queued submission without changing its status", async () => {
				...
				expect(await store.submissions.requestSessionAbort(sessionKey)).toEqual(["dispatch-1"]);
				expect(await store.submissions.getSubmission("dispatch-1")).toMatchObject({
					status: "queued",
			...
			it("keeps the first abort timestamp when the request is repeated", async () => {
				...
				await store.submissions.requestSessionAbort(sessionKey);
				expect((await store.submissions.getSubmission("dispatch-1"))?.abortRequestedAt).toBe(stampedAt);
			});
			it("leaves settled submissions untouched and does not resurrect them", async () => {
				...
				expect(await store.submissions.claimSubmission(claim("dispatch-1", "attempt-2"))).toBeNull();
			});
			it("returns an empty array for a session with no unsettled submissions", async () => {
				expect(await (await create()).submissions.requestSessionAbort("no-such-session")).toEqual([]);
			});
		});
		describe("direct settlement obligation", () => {
			const settlementRecord = (outcome) => ({
				v: 1,
				id: "direct-1:settled",
			...
			it("reserves a canonical settlement only for the owning direct attempt", async () => {
				...
				expect(await store.submissions.reserveSubmissionSettlement(attempt("direct-1", "stale"), {
					recordId: record.id,
					record
				})).toBeNull();
				expect(await store.submissions.reserveSubmissionSettlement(attempt("direct-1", "attempt-1"), {
					recordId: record.id,
					record
				})).toEqual({
					submissionId: "direct-1",
				...
				expect(await store.submissions.getSubmission("direct-1")).toMatchObject({ status: "terminalizing" });
			});
			it("replays an exact obligation and rejects a conflicting settlement payload", async () => {
				...
				expect(await store.submissions.reserveSubmissionSettlement(ref, {
					recordId: completed.id,
					record: completed
				})).toEqual(first);
				expect(await store.submissions.reserveSubmissionSettlement(ref, {
			...
			it("leaves dispatch settlement behavior unchanged", async () => {
				...
				await store.submissions.claimSubmission(claim("dispatch-1", "attempt-1"));
				const record = settlementRecord("completed");
				expect(await store.submissions.reserveSubmissionSettlement(attempt("dispatch-1", "attempt-1"), {
					recordId: record.id,
					record
				})).toBeNull();
				expect(await store.submissions.completeSubmission(attempt("dispatch-1", "attempt-1"))).toBe(true);
			});
		});
		describe("durability", () => {
			...
			it("sets attempt_count to 1 and applies system defaults at claim time", async () => {
				...
				expect(claimed.timeoutAt).toBeGreaterThanOrEqual(before + 60 * 6e4);
			});
			it("applies custom durability when input is marked applied", async () => {
				const store = await create();
				await admitDispatchReady(store, dispatchInput());
				const customTimeout = Date.now() + 360 * 6e4;
				await store.submissions.claimSubmission(claim("dispatch-1", "attempt-1"));
			...
			it("increments attempt_count on recovery via replaceSubmissionAttempt", async () => {
				...
				expect(await store.submissions.replaceSubmissionAttempt(attempt("dispatch-1", "attempt-1"), "attempt-2")).toMatchObject({
					attemptCount: 2,
					attemptId: "attempt-2"
				});
			});
		...
		describe("replaceSubmissionAttempt()", () => {
			...
			it("returns null without writing when the attempt no longer owns the submission", async () => {
				...
				await admitDispatchReady(store, dispatchInput());
				await store.submissions.claimSubmission(claim("dispatch-1", "attempt-1"));
				expect(await store.submissions.replaceSubmissionAttempt(attempt("dispatch-1", "attempt-stale"), "attempt-2")).toBeNull();
				expect(await store.submissions.getSubmission("dispatch-1")).toMatchObject({
					attemptId: "attempt-1",
					attemptCount: 1
				});
			...
			it("installs the new lease when one is supplied", async () => {
				...
				await admitDispatchReady(store, dispatchInput());
				await store.submissions.claimSubmission(claim("dispatch-1", "attempt-1"));
				const leaseExpiresAt = Date.now() + 6e4;
				expect(await store.submissions.replaceSubmissionAttempt(attempt("dispatch-1", "attempt-1"), "attempt-2", {
					ownerId: "owner-2",
					leaseExpiresAt
				})).toMatchObject({
					attemptId: "attempt-2",
		...
		describe("attempt markers", () => {
			...
			it("deletes only the marker matching both submission and attempt", async () => {
				...
				expect(await store.submissions.listAttemptMarkers()).toEqual([expect.objectContaining({
					submissionId: "dispatch-1",
					attemptId: "attempt-2"
				})]);
			});
			it("silently handles deleting nonexistent markers", async () => {
				await expect((await create()).submissions.deleteAttemptMarker(attempt("missing", "attempt-1"))).resolves.toBeUndefined();
			});
		});
		...
		describe("listExpiredSubmissions()", () => {
			...
			it("returns empty when no submissions exist", async () => {
				expect(await (await create()).submissions.listExpiredSubmissions()).toHaveLength(0);
			});
		});
		describe("edge cases", () => {
			it("reports no unsettled submissions initially", async () => {
				expect(await (await create()).submissions.hasUnsettledSubmissions()).toBe(false);
			});
			it("getSubmission returns null for unknown ids", async () => {
				expect(await (await create()).submissions.getSubmission("nonexistent")).toBeNull();
			});
		});
	});
}
//#endregion
export { defineAttachmentStoreContractTests, defineConversationStreamStoreContractTests, defineEventStreamStoreContractTests, defineRunStoreContractTests, defineStoreContractTests };
