...
//#region src/test-utils/define-event-stream-store-contract-tests.ts
function defineEventStreamStoreContractTests(label, backend) {
	describe(label, () => {
		let cleanup;
		...
		it("returns the same offset when appendEventOnce retries the same keyed event", async () => {
			...
			expect(await store.appendEventOnce("runs/test", "terminal-1", { index: 0 })).toBe(first);
			expect(await store.readEvents("runs/test")).toMatchObject({ events: [{
				data: { index: 0 },
				offset: first
			}] });
		});
		it("rejects a conflicting appendEventOnce payload without appending", async () => {
			...
			await expect(store.appendEventOnce("runs/test", "terminal-1", { index: 1 })).rejects.toThrow("conflicting payload");
		...
		it("allocates distinct offsets for concurrent appendEventOnce calls", async () => {
			const store = await create();
			await store.createStream("runs/test");
			const offsets = await Promise.all([store.appendEventOnce("runs/test", "event-1", { index: 0 }), store.appendEventOnce("runs/test", "event-2", { index: 1 })]);
		...
		it("pages through a stream and reports the tail", async () => {
			const store = await create();
			await store.createStream("runs/test");
			for (let index = 0; index < 3; index++) await store.appendEvent("runs/test", { index });
			const firstPage = await store.readEvents("runs/test", {
				offset: "-1",
				limit: 2
			});
			...
			expect(await store.readEvents("runs/test", {
		...
		it("marks an exactly-limit page ending at the tail as up to date", async () => {
			const store = await create();
			await store.createStream("runs/test");
			for (let index = 0; index < 4; index++) await store.appendEvent("runs/test", { index });
			const partial = await store.readEvents("runs/test", {
				offset: "-1",
				limit: 2
			});
		...
		it("returns null metadata for missing streams", async () => {
			expect(await (await create()).getStreamMeta("runs/missing")).toBeNull();
		});
...
//#endregion
export { defineEventStreamStoreContractTests };
