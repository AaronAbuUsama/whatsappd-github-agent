import { n as ToolDefinition, s as JsonValue } from "./tool-types-CcKIl663.mjs";
import * as v from "valibot";
import { ImageContent, Model } from "@earendil-works/pi-ai";
import { MiddlewareHandler } from "hono";
import { AgentMessage, AgentTool, ThinkingLevel } from "@earendil-works/pi-agent-core";

//#region src/action.d.ts
type ActionInputSchema = v.GenericSchema<Record<string, unknown>, unknown>;
type ActionOutputSchema = v.GenericSchema<any, NonNullable<unknown> | null>;
type ActionContext<S extends ActionInputSchema | undefined> = {
  readonly harness: FlueHarness;
  readonly log: FlueLogger;
} & (S extends ActionInputSchema ? {
  readonly input: v.InferOutput<S>;
} : Record<never, never>);
type ActionRunResult<S extends ActionOutputSchema | undefined> = S extends ActionOutputSchema ? v.InferInput<S> : JsonValue | undefined;
interface ActionDefinition<TInput extends ActionInputSchema | undefined = ActionInputSchema | undefined, TOutput extends ActionOutputSchema | undefined = ActionOutputSchema | undefined> {
...
type ActionInput<TAction extends ActionDefinition> = TAction extends ActionDefinition<infer TInput, any> ? TInput extends ActionInputSchema ? v.InferInput<TInput> : never : never;
type ActionOutput<TAction extends ActionDefinition> = TAction extends ActionDefinition<any, infer TOutput> ? TOutput extends ActionOutputSchema ? v.InferOutput<TOutput> : unknown : never;
type ActionOptions<TInput extends ActionInputSchema | undefined, TOutput extends ActionOutputSchema | undefined> = {
  name: string;
  description: string;
  input?: TInput;
  output?: TOutput;
...
declare function defineAction<const TInput extends ActionInputSchema | undefined = undefined, const TOutput extends ActionOutputSchema | undefined = undefined>(options: ActionOptions<TInput, TOutput>): ActionDefinition<TInput, TOutput>;
//#endregion
...
type AgentRouteHandler = MiddlewareHandler;
type WorkflowRouteHandler = MiddlewareHandler;
type WorkflowRunsHandler = MiddlewareHandler;
 ...
 */
type DirectAgentImage = PromptImage & {
  filename?: string;
};
...
interface AgentInitializerContext<TEnv = Record<string, any>> {
...
interface PackagedSkillDirectory {
  readonly id: string;
  readonly name: string;
  readonly description: string;
...
interface AgentDefinition<TEnv = Record<string, any>> {
...
interface FlueSession {
   ...
   */
  skill<S extends v.GenericSchema>(skill: SkillReference | string, options: SkillOptions<S> & {
    result: S;
  }): CallHandle<PromptResultResponse<v.InferOutput<S>>>;
...
interface OperationOptions<S extends v.GenericSchema | undefined = undefined> {
  ...
  tools?: ToolDefinition[];
  /** Framework-owned tools exposed to the model for this operation. */
  frameworkTools?: {
    /** Disable model-facing task delegation while preserving programmatic session.task(). */
    task?: boolean;
  };
...
interface PromptOptions<S extends v.GenericSchema | undefined = undefined> extends OperationOptions<S> {
...
interface SkillOptions<S extends v.GenericSchema | undefined = undefined> extends OperationOptions<S> {
...
interface TaskOptions<S extends v.GenericSchema | undefined = undefined> extends OperationOptions<S> {
 ...
 * {@link FlueEvent}.
 */
type FlueEventInput = FlueEventVariant & {
  runId?: string;
  instanceId?: string;
  dispatchId?: string;
  submissionId?: string;
...
type FlueEvent = FlueEventInput & {
  /** Durable event-format version. Readers branch on this when the format changes. */v: 3;
  eventIndex: number;
  timestamp: string;
};
declare const FLUE_EVENT_SCHEMA_REVISION = 3;
type FlueObservation = FlueEvent & FlueObservationDetail;
/**
 ...
 */
type AttachedAgentEvent = Exclude<FlueEvent, {
  type: 'run_start';
} | {
  type: 'run_resume';
} | {
...
type FlueEventInputCallback = (event: FlueEventInput, observation?: FlueObservationDetail) => void | Promise<void>;
type FlueEventCallback = (event: FlueEvent) => void | Promise<void>;
type AttachedAgentEventCallback = (event: AttachedAgentEvent) => void | Promise<void>;
//#endregion
export { PromptUsage as $, FlueSessions as A, LlmUserMessage as B, FlueEventInputCallback as C, FlueObservation as D, FlueLogger as E, LlmThinkingContent as F, NamedAgentDispatchRequest as G, ModelRequestInfo as H, LlmTool as I, PromptImage as J, PackagedSkillDirectory as K, LlmToolCall as L, LlmImageContent as M, LlmMessage as N, FlueObservationDetail as O, LlmTextContent as P, PromptResultResponse as Q, LlmToolResultMessage as R, FlueEventInput as S, FlueHarness as T, ModelRequestInput as U, ModelRequest as V, ModelResponse as W, PromptOptions as X, PromptModel as Y, PromptResponse as Z, FLUE_EVENT_SCHEMA_REVISION as _, ActionOutput as _t, AgentProfile as a, ShellResult as at, FlueEventCallback as b, AttachedAgentEvent as c, SkillReference as ct, BashLike as d, WorkflowRouteHandler as dt, SandboxFactory as et, CallHandle as f, WorkflowRunsHandler as ft, DurabilityConfig as g, ActionInputSchema as gt, DispatchReceipt as h, ActionInput as ht, AgentInitializerContext as i, ShellOptions as it, LlmAssistantMessage as j, FlueSession as k, AttachedAgentEventCallback as l, TaskOptions as lt, DirectAgentPayload as m, ActionDefinition as mt, AgentDefinition as n, SessionToolFactory as nt, AgentRouteHandler as o, Skill as ot, CompactionConfig as p, ActionContext as pt, PackagedSkillFile as q, AgentDispatchRequest as r, SessionToolFactoryOptions as rt, AgentRuntimeConfig as s, SkillOptions as st, AgentConfig as t, SessionEnv as tt, BashFactory as u, ThinkingLevel as ut, FileStat as v, ActionOutputSchema as vt, FlueFs as w, FlueEventContext as x, FlueEvent as y, defineAction as yt, LlmTurnPurpose as z };