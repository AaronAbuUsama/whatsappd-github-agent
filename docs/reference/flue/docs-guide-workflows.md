...
title: Workflows | Flue
image: https://flueframework.com/docs/og4.jpg
---

# Workflows

Last updated Jun 22, 2026 [ View as Markdown ](https://flueframework.com/docs/guide/workflows/index.md) 

Workflows are finite, inspectable operations for background jobs, document transformations, reviews, and CI tasks. Use an [agent](https://flueframework.com/docs/guide/building-agents/) instead when work should continue across messages.
...
This defines the `summarize` workflow. Each invocation validates the supplied text, asks the model to summarize it, and returns a validated `{ summary }` result. Use this pattern for finite work that should have its own run, result, and event history. See the [Workflow API](https://flueframework.com/docs/api/workflow-api/) for the complete definition contract.

## Use a reusable Action

Define the workflow inline as shown above for the ordinary case. If the workflow should run an existing [Action](https://flueframework.com/docs/guide/actions/), bind that Action to its agent:
...
The Action owns the input, output, and handler, so the workflow does not repeat them. See [Actions](https://flueframework.com/docs/guide/actions/) for when and how to define one.
...
A session can also run skills, delegate tasks, and produce schema-backed structured results. See [Agent API](https://flueframework.com/docs/api/agent-api/), [Skills](https://flueframework.com/docs/guide/skills/), [Subagents](https://flueframework.com/docs/guide/subagents/), and [Sandboxes](https://flueframework.com/docs/guide/sandboxes/).
